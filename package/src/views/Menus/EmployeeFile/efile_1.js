...
import { // Material-UI (MUI) components for UI structure and styling
  Box,
  Grid,
  Paper,
  Typography,
...
import { // Material-UI Icons for navigation and section identification
  Translate,
...
import { // Custom encryption/decryption utilities for secure API communication
  decryptData,
  encryptPayloadForGo,
...
import { // Imports for sub-components used in specific section renders
...
const EmployeeEfile = ({
  ...
  // ✅ TYPE DEBUGGING - Console logs to trace initialization parameters
  console.log("=== EmployeeEfile Type Debug ===");
 ...
 const [loading, setLoading] = useState(false); // Global loading state for initial data fetch (kept for context)
  ...
  const checkApiResponseForAccess = (apiResponse) => {
    console.log("🔍 Checking API response for E-File access:", apiResponse);
    ...
    if (Array.isArray(apiResponse)) {
      ...
      console.log("❌ API returned no valid data structure");
    ...
    console.log("📋 All available modules:", modulesArray);
 ...
 /**
  ...
  const fetchRoleData = async (role, session_id) => {
    console.log("🚀 fetchRoleData called with:", {
      role: role,
    ...
    try {
      ...
      console.log("🧩 Final payload being encrypted:", {
        token: payloadToEncrypt.token,
        session_id: session_id,
        role_name: payloadToEncrypt.role_name,
      });
   // Decryption Logic (robust handling of pre-decrypted or encrypted data)
      ...
      if (responseData.Data && Array.isArray(responseData.Data)) {
        console.log("✅ Role data is already decrypted array");

        const modules = responseData.Data;
        ...
        console.log("✅ Role API completed successfully:", {
          hasAccess,
          moduleCount: filteredModules?.length ?? 0,
        });
      ...
      try {
        ...
        console.error("❌ Decrypted data is not valid JSON:", parseError);
      ...
      console.error("❌ Role API Fetch Error:", err);
  ...
  const fetchEmployeeDetails = async () => {
    console.log("🚀 Starting fetchEmployeeDetails for adminefile...");
    ...
    try {
      ...
      if (Array.isArray(decryptedData.Data)) {
        ...
        console.warn(
          "⚠️ Expected Data to be an array, got:",
          decryptedData.Data
        );
      }
  ...
  // All other API functions (fetchPersonalDetails, fetchAppointmentDetails, etc.)
  // remain the same as your existing code...
  /**
  ...
  const fetchPersonalDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchPersonalDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      if (!pureEmployeeId) {
        console.warn("⛔ Skipping fetch — invalid employee_id");
        return;
      }
      ...
      console.log("📦 Payload to encrypt:", payloadToEncrypt);
      ...
      console.log("📤 Sending EmployeeBasicDetails API Request...");
      ...
      if (
        ...
        console.log(
          "✅ Personal details response appears to be already decrypted"
        );
      ...
      if (!encryptedData) {
        console.warn("⚠️ No encrypted data found");
        throw new Error("No data payload found in response");
      }
      ...
      if (typeof encryptedData === "string") {
        try {
          console.log("🔓 Attempting standard decryption...");
          ...
          try {
            ...
            console.log("✅ Standard decryption and JSON parse successful");
          } catch (jsonError) {
            console.warn(
          ...
          console.warn("❌ Standard decryption failed:", decryptError.message);
          ...
          try {
            ...
            console.log("🔓 Using encrypted data as plain object...");
      ...
      console.log("🔓 Final data after decryption attempts:", finalData);
      ...
      console.log("🎯 Final personal data:", personalData);
      ...
      console.error("❌ Personal Details API Fetch Error:", err);
      setError((prev) => ({
        ...prev,
  ...
  const fetchAppointmentDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchAppointmentDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      console.log("📤 Sending EmployeeAppointmentDetails API Request...");
      ...
      if (
        ...
        console.log("✅ Appointment response appears to be already decrypted");
      ...
      console.error("❌ Appointment API Fetch Error:", err);
      setError((prev) => ({
        ...prev,
        appointment:
  ...
  const fetchEducationDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchEducationDetails...", { employeeid });
    ...
    try {
      ...
      console.log("📤 Sending encrypted request to API...");
      ...
      // Parse the response
      const responseData = await response.json();
      console.log("📥 Response received, Data type:", typeof responseData.Data);
      ...
      if (decryptedData.Status !== 200) {
        console.warn(`⚠️ API returned non-200 status: ${decryptedData.Status}`);
        throw new Error(
      ...
      if (
        ...
        educationData = decryptedData.Data.education_details;
        console.log(
          "✅ Found education_details in Data.education_details (Old nested format)"
        );
      }
      // 2. Check for the NEW structure: { Data: [...] }
      else if (decryptedData.Data && Array.isArray(decryptedData.Data)) {
        educationData = decryptedData.Data;
        console.log(
          "✅ Found education_details at Data root (NEW direct array format)"
        );
      }
      // 3. Check for the root structure: { education_details: [...] }
        ...
        educationData = decryptedData.education_details;
        console.log("✅ Found education_details at root level");
      }
      // 4. Fallback: If the decrypted data itself is the array
      else if (Array.isArray(decryptedData)) {
        educationData = decryptedData;
        console.log("✅ Decrypted data is already an array");
      } else {
        console.warn(
          "⚠️ Education data not found in expected array structure. Initializing as empty."
        );
        educationData = []; // Ensure it remains an array
      }
      // -----------------------------------------------------------
      // END OF CHANGES
      // -----------------------------------------------------------
      ...
      if (!Array.isArray(educationData)) {
        console.error(
          "❌ education_details is not an array:",
          typeof educationData
        );
        educationData = [];
      }

      console.log("🎯 Education records found:", educationData.length);

      // Log first record structure (if exists)
      if (educationData.length > 0) {
        console.log("📋 First record fields:", Object.keys(educationData[0]));
      }
      ...
      console.log("✅ Education data stored in state successfully");
      return educationData;
    } catch (err) {
      console.error("❌ Education API Fetch Error:", err);

      // Enhanced error logging
      if (err.isSessionRelated) {
        console.error("🔐 Session-related error detected");
      }
  ...
  const fetchDependentDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchDependentDetails...", { employeeid });
    ...
    try {
      ...
      if (
        ...
        console.log("✅ Found dependents in Data.dependents (Nested format)");
      }
      // 2. Check for the direct array structure: { Data: [...] } (Potential format)
      else if (decryptedData.Data && Array.isArray(decryptedData.Data)) {
        dependentData = decryptedData.Data;
        console.log("✅ Found dependents at Data root (Direct Array Format)");
      }
      // 3. Check for the root structure: { dependents: [...] }
        ...
        console.log("✅ Found dependents at root level");
      }
      // 4. Fallback: If the decrypted data itself is the array
      else if (Array.isArray(decryptedData)) {
        ...
        console.warn(
          "⚠️ Dependent data not found in expected array structure. Initializing as empty."
        );
        dependentData = [];
      }
      // -----------------------------------------------------------
      ...
      if (!Array.isArray(dependentData)) {
        console.error("❌ dependents is not an array:", typeof dependentData);
        dependentData = [];
      }

      console.log("🎯 Dependent records found:", dependentData.length);

      // Log first record structure (if exists)
      if (dependentData.length > 0) {
        console.log("📋 First record fields:", Object.keys(dependentData[0]));
      }
      ...
      console.log("✅ Dependent data stored in state successfully");
      return dependentData;
    } catch (err) {
      console.error("❌ Dependent API Fetch Error:", err);
  ...
  const fetchLanguageDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchLanguageDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      console.log("📤 Sending EmployeeLanguageDetails API Request..."); // CORRECTED
      ...
      if (
        ...
        console.log("✅ Language response appears to be already decrypted");
      ...
      console.error("❌ Language API Fetch Error:", err); // CORRECTED
      setError((prev) => ({
        ...prev,
  ...
  const fetchDocumentDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting Documents...", {
      employeeid,
    });
    ...
    try {
      ...
      console.log("📤 Sending EmployeeExperienceDetails API Request...");
      ...
      console.log("🎯 Final experience data:", documentData);
  ...
  const fetchNomineeDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchNomineeDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      console.log("📤 Sending EmployeeNomineeDetails API Request...");
      ...
      console.log(
        "✅ Decryption successful. Decrypted Status:",
      ...
      // Log the extracted data keys for debugging
      console.log("🎯 Extracted Nominee Data Keys:", Object.keys(nomineeData));
      ...
      console.error("❌ Nominee API Fetch Error:", err);
      setError((prev) => ({
        ...prev,
  ...
  const fetchContactDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchContactDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      console.log("📤 Sending EmployeeContactDetails API Request...");
      ...
      if (
        ...
        console.log("✅ Contact response appears to be already decrypted");
  ...
  const fetchExperienceDetails = async (employeeid = effectiveEmployeeId) => {
    console.log("🚀 Starting fetchExperienceDetails...", {
      employeeid,
    });
    ...
    try {
      ...
      if (decryptedData.Data && Array.isArray(decryptedData.Data)) {
        ...
        console.log(
          "✅ Found experience details at Data root (New array format)"
        );
      } else if (
        ...
        console.log(
          "✅ Found experience details in Data.experience_details (Old nested format)"
        );
      } else if (Array.isArray(decryptedData)) {
        ...
        console.warn(
          "⚠️ Experience data not found in expected array structure."
        );
        experienceData = [];
      }

      console.log("🎯 Experience records found:", experienceData.length);
  ...
  const fetchSectionData = async (
    ...
    console.log("🚀 fetchSectionData called:", {
      sectionId,
      employeeid,
      effectiveEmployeeId,
    });
    ...
    try {
      ...
      switch (sectionId) {
        ...
        case "contact":
          ...
          break;
        default:
          console.warn(`❌ No specialized function for section: ${sectionId}`);
          throw new Error(`No data fetcher available for ${sectionId}`);
      }

      console.log(`✅ Successfully fetched data for ${sectionId}`);
    } catch (err) {
      console.error(`❌ Error fetching ${sectionId} data:`, err);
      setError((prev) => ({
        ...prev,
  ...
  const fetchSectionDataForAll = async () => {
    ...
    console.log(
      "📥 Fetching data for all accessible sections:",
      accessibleSections.map((s) => s.id)
    );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        "🎯 Role API successful - Auto-populating employee ID:",
        initialEmployeeId
      );
      ...
      console.log("✅ Employee ID auto-populated after role API success");
    }
  }, [
    roleCheckLoading,
    hasEfileAccess,
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        if (!jwtToken || !sessionId || !userRole) {
          console.warn("⚠️ Required params missing. Aborting initialization.");
          setHasEfileAccess(false);
        ...
        console.log("🚀 Step 1: Fetching Role-Based Modules...");
        ...
        if (!hasAccess) {
          console.warn("⛔ No access. Stopping sequence.");
          setHasEfileAccess(false);
          return;
        }
    ...
    if (effectiveUserRole && effectiveSessionId) {
      ...
      console.warn("⚠️ Cannot initialize - missing params");
      setRoleCheckLoading(false);
    }
 ...
 // Effect to trigger fetching all accessible sections once the employee ID is set/changed
  useEffect(() => {
    ...
    console.log("🚀 Fetching section data for:", form.employeeid);
  ...
  const handleSectionClick = async (sectionId) => {
    ...
    console.log(" Current access details:", getAccessDetails());

    // Comprehensive validation
    const validation = validateSectionAccess(sectionId);

    if (!validation.isValid) {
      console.error(` Validation failed for ${sectionId}:`, validation.errors);
    ...
    console.log(`Validation passed for ${sectionId}`);
    ...
    if (!apiSectionData[sectionId] && !apiSectionLoading[sectionId]) {
      console.log(` Fetching data for section: ${sectionId}`);
      await fetchSectionData(sectionId, form.employeeid);
    } else {
      console.log(` Section data already available or loading: ${sectionId}`);
    }
  };
  ...
  const showDetailedAccessError = (validation) => {
    ...
    // You can show this in a dialog or more prominent alert
    console.error(" Detailed access error:", detailedMessage);
  ...
  /**
   * Special handler for Personal Details section with enhanced validation and console logging
   */
  const handlePersonalDetailsClick = async () => {
    const sectionId = "personal";
    console.group("🔐 PERSONAL DETAILS - Screen Wise Validation");

    console.log("🎯 Section Clicked: Personal Details");
    ...
    if (!validation.isValid) {
      console.error("❌ Personal Details access denied - Validation Failed");
      console.log("📊 Validation Errors:", validation.errors);
      console.log("🔍 Missing Fields:", validation.missingFields);

      // Show access details for debugging
      const accessDetails = getAccessDetails();
      console.log("👤 User Access Details:", accessDetails);

      // Check specific screen access
      console.log("🔎 Screen Access Check:");
      console.log("- Role API Failed:", roleApiFailed);
      console.log("- Has E-File Access:", hasEfileAccess);
      console.log("- Employee Selected:", isEmployeeSelected());
      console.log("- Employee ID:", form.employeeid);
      console.log("- JWT Token:", !!Cookies.get("HRToken"));
      console.log("- Session ID:", !!Cookies.get("session_id"));
      console.log("- User Role:", Cookies.get("selectedRole"));

      // Check module access specifically for Personal
      console.log(
        "📋 Available Modules:",
        moduleList.map((m) => m.module_name)
      );
      const hasPersonalModule = moduleList.some((module) =>
        module.module_name?.toLowerCase().includes("personal")
      );
      console.log("- Personal Module Found:", hasPersonalModule);
    ...
    console.log("✅ Personal Details access granted - Validation Passed");
   ...
   */
  /**
   * Enhanced validation specifically for Personal Details with detailed console logging
   */
  const validatePersonalDetailsAccess = () => {
    console.group("🧪 Personal Details Validation Steps");
    ...
    console.log("1. Checking Role API Status...");
    if (roleApiFailed) {
      console.error("❌ Role API failed");
      validation.isValid = false;
      validation.errors.push(
        "Role verification failed - contact administrator"
      );
    } else {
      console.log("✅ Role API is working");
    }

    // Step 2: Check if user has E-File access
    console.log("2. Checking E-File Access...");
    if (!hasEfileAccess) {
      console.error("❌ No E-File access");
      validation.isValid = false;
      validation.errors.push("Access denied to Employee E-File");
    } else {
      console.log("✅ Has E-File access");
    }

    // Step 3: Check if employee is selected
    console.log("3. Checking Employee Selection...");
    if (!isEmployeeSelected()) {
      ...
      console.log("✅ Employee selected:", form.employeeid);
    }

    // Step 4: Check if Personal section is accessible for this role
    console.log("4. Checking Section Access...");
    const accessibleSections = getAccessibleSections();
    ...
    if (!hasSectionAccess) {
      console.error("❌ Personal section not accessible");
      console.log(
        "📋 Accessible Sections:",
        accessibleSections.map((s) => s.id)
      );
      ...
      console.log("✅ Personal section is accessible");
    }
    ...
    if (!sessionId) {
      ...
      console.log("✅ Session ID present");
    }

    if (!userRole) {
      console.error("❌ User role not selected");
      validation.isValid = false;
    ...
    console.log("6. Checking Screen/Category Access...");
    ...
    if (section) {
      ...
      if (!hasCategoryAccess) {
        console.error("❌ Personal category access denied");
        console.log(
          "📋 Available modules:",
          moduleList.map((m) => m.module_name)
        );
        ...
        console.log("✅ Personal category access granted");
      }
    }
  ...
  const showPersonalDetailsAccessError = (validation) => {
    let errorTitle = "Cannot Access Personal Details";
    ...
    console.group("Personal Details Access Error");
    console.error("Validation Errors:", validation.errors);
    console.error("Access Details:", accessDetails);
    console.groupEnd();
  };
  ...
  const getAccessibleSections = () => {
    console.group("🎯 Get Accessible Sections Analysis");

    // If role API failed, return all sections (they will be disabled)
    if (roleApiFailed) {
      console.log("❌ Role API failed - showing all sections as disabled");
      console.groupEnd();
      return sections;
    }
    ...
    if (!hasEfileAccess || !moduleList || moduleList.length === 0) {
      console.log("❌ No E-File access or no modules available");
      console.groupEnd();
      return [];
    }

    console.log("📋 Available modules from API:", moduleList);
    console.log(
      "🔍 Module Names:",
      moduleList.map((m) => m.module_name)
    );
    ...
    console.log("🔍 Step 1: Direct Mapping");
    moduleList.forEach((module) => {
      ...
      if (moduleName && mapping[moduleName]) {
        ...
        console.log(
          `✅ Direct mapping: ${module.module_name} -> ${mapping[moduleName]}`
        );
      }
    });

    // Method 2: Fuzzy matching for remaining modules
    console.log("🔍 Step 2: Fuzzy Matching");
    moduleList.forEach((module) => {
      const moduleName = module.module_name?.toLowerCase();
      ...
      for (const section of sections) {
        if (
          ...
          console.log(`🔍 Fuzzy match: ${module.module_name} -> ${section.id}`);
          matched = true;
          break;
        }
      }
      ...
      if (!matched) {
        console.log(`❓ No match found for module: ${module.module_name}`);
      }
    });

    console.log("🎯 Final accessible section IDs:", accessibleSectionIds);
    ...
    console.log(
      "📊 Accessible sections:",
      accessibleSections.map((s) => s.label)
    );
    console.groupEnd();
 ...
 */
            ...
            basic: {
              ...
              fields: [
                ...
                },
                {
                  ...
                  value: personalDetails.birth_district || "",
                  type: "text",
                },
                {
                  label: "Birth Place",
                ...
                },
                {
                  ...
                  value: personalDetails.caste_category
                    ? personalDetails.caste_category.trim()
                    : "",
                  type: "text",
                },
                {
                  label: "Emergency Contact",
                  value: personalDetails.emergency_contact_no || "",
                  type: "text",
                },
                {
                  label: "Mobile No",
                ...
                },
                {
                  ...
                  value: personalDetails.is_physically_challenged || "",
                  type: "text",
                },
                {
                  label: "Disability Percentage",
                  value: personalDetails.percentage_of_disability || "",
                  type: "text",
                },
                {
                  label: "Nature of Disability",
                  value: personalDetails.nature_of_disability || "",
                  type: "text",
                },
                {
                ...
                {
                  label: "Aadhaar No",
                  value: personalDetails.aadhaar_no || "",
                  type: "text",
                },
                {
                  label: "Mother Tongue",
                  value: personalDetails.mother_tongue || "",
                  type: "text",
                },
                {
                  label: "Identification Marks",
                  value: personalDetails.identification_marks || "",
                  type: "text",
                },
                {
                  label: "PAN Card No",
                  value: personalDetails.pan_card_no || "",
                  type: "text",
                },
              ],
            },
  ...
  // Log transformation for debugging
        console.log(
          "✅ Personal Data Transformation Complete:",
          transformedData
        );
          ...
          content: {
            employment: {
              ...
              fields: [
                ...
                },
                {
                  ...
                  value: employmentInfo.employee_group || "",
                  type: "text",
                },
                {
                  label: "Employee Name",
                  value: employmentInfo.employee_name || "",
                  type: "text",
                },
                {
                  label: "Employee Status",
                  value: employmentInfo.employee_status || "",
                  type: "text",
                },
                {
                  label: "Employee Type",
                ...
                {
                  label: "Is Active",
                  value: employmentInfo.is_active || "",
                  type: "text",
                },
                {
                  label: "Route To",
                  value: employmentInfo.route_to || "",
                  type: "text",
                },
                {
                  label: "Section",
            ...
            details: {
              ...
              // Employment detailed information
                {
                  label: "Pay Info",
                  value: employmentDetails.pay_info || "",
                  type: "text",
                },
                {
                  label: "Basic Pay", // Fixed label
                  value: employmentDetails.basic_pay || "",
                  type: "text",
                },
                {
                  label: "Non Practice Pay",
                  value: employmentDetails.non_practice_pay || "",
                  type: "text",
                },
                {
                  label: "Pay Band",
                  value: employmentDetails.name_of_pay_band || "",
                  type: "text",
                },
                {
                  label: "Date of Joining",
                  value: employmentDetails.date_of_joining || "",
                  type: "date",
                },
                {
                  label: "Date of Confirmation",
                  value: employmentDetails.date_of_confirmation || "",
                  type: "date",
                },
                {
                  label: "Date of Retirement",
                  value: employmentDetails.date_of_retirement || "",
                  type: "date",
                },
                {
                  label: "Office Room No",
                  value: employmentDetails.office_room_no || "",
                  type: "text",
                },
                {
                  label: "Office Extension No",
                  value: employmentDetails.office_extension_no || "",
                  type: "text",
                },
              ],
            },
      ...
      case "education":
        console.log("🔍 RAW apiData for education:", apiData);
        ...
        if (apiData?.Data && Array.isArray(apiData.Data)) {
          ...
          console.log("✅ Using Data array structure from raw response");
        }
        // 2. If state stored the raw response (old nested structure)
          ...
          console.log(
            "✅ Using Data.education_details array structure from raw response"
          );
        }
        // 3. If state stored the extracted array directly (MOST LIKELY)
        else if (Array.isArray(apiData)) {
          educationDetails = apiData;
          console.log("✅ Using direct array structure (from state)");
        } else {
          console.warn("⚠️ No education details found in expected structure");
          educationDetails = [];
        }

        console.log("📚 Education details array:", educationDetails);
        console.log("📚 Number of education records:", educationDetails.length);
       ...
       */
        const getValue = (value, defaultText = "Not specified") => {
        ...
        // If no education data found
        if (educationDetails.length === 0) {
          return {
            title: sectionTitle,
            content: [], // Return an empty array for the table
          };
        }

        // Transform each education record into a FLAT object suitable for CommonDataTable
        const transformedEducation = educationDetails.map((edu) => {
        ...
        console.log(
          "🎯 Transformed education data (Flat Structure):",
      ...
      case "experience":
        console.log("🔄 Transforming experience data:", apiData);

        /**
       * Safely extracts and formats experience field values with date conversion
       * @param {any} value - Raw field value
       * @param {string} defaultText - Fallback text
       * @returns {string} Formatted value with date conversion
       */
        const getData = (value, defaultText = "Not specified") => {
          if (value === null || value === undefined || value === "") {
            return defaultText;
          }
        ...
        // Ensure apiData is an array of records
        ...
        if (experienceRecords.length === 0) {
          return {
            title: sectionTitle,
            content: [], // Return empty array for the table
          };
        }

        // 🎯 CRITICAL FIX: Transform to an array of FLAT objects for CommonDataTable
        const transformedExperience = experienceRecords.map((record) => {
          return {
        ...
        console.log(
          "✅ Transformed experience data (Flat Structure):",
          transformedExperience
        );

        return {
          title: sectionTitle,
          content: transformedExperience, // Pass the array of flat objects
        };

      case "dependents": {
        console.log("🔄 Transforming dependents data:", apiData);
        ...
        if (dependents.length === 0) {
          console.log("⚠️ No dependents data available");
          return {
        ...
        console.log("🎯 Final transformed content (Raw Array):", dependents);
      ...
      case "contact":
        console.log("🔍 RAW apiData for contact:", apiData);
          ...
          content: {
            current: {
              ...
              fields: [
                ...
                {
                  label: "City/Place",
                  value: currentAddress.city || "Not specified",
                  type: "text",
                },
                {
                  label: "District",
                  value: currentAddress.district || "Not specified",
                  type: "text",
                },
                {
                  label: "State",
                  value: currentAddress.state || "Not specified",
                  type: "text",
                },
                {
                  label: "Country",
                  value: currentAddress.country || "Not specified",
                  type: "text",
                },
                {
                  label: "PIN Code",
            ...
            permanent: {
              ...
              fields: [
                ...
                {
                  label: "City/Place",
                  value: permanentAddress.city || "Not specified",
                  type: "text",
                },
                {
                  label: "District",
                  value: permanentAddress.district || "Not specified",
                  type: "text",
                },
                {
                  label: "State",
                  value: permanentAddress.state || "Not specified",
                  type: "text",
                },
                {
                  label: "Country",
                  value: permanentAddress.country || "Not specified",
                  type: "text",
                },
                {
                  label: "PIN Code",
      ...
      case "language":
        console.log("🔍 RAW apiData for language:", apiData);
        ...
        console.log("🔍 Hindi Proficiency Data:", hindiProficiencyData);
 ...
 */
  ...
  const renderTableSection = (sectionData, color, columnsConfig = null) => {
    ...
    if (isDetailList) {
      console.log("Rendering as Detail List (Education/Card View)");
    ...
    // ------------------------------------------------------------------------
    console.log("Rendering as Common Data Table (Simple Array)");
    ...
    try {
      ...
      console.error("Error rendering CommonDataTable:", err);
      return (
        <Box sx={{ textAlign: "center", py: 4 }}>
          <Alert
            severity="error"
 ...
 */
  ...
  const renderCompactProfileSection = (sectionData, color) => {
    ...
    console.log(
      "RENDER DEBUG: Section Data Received (dependents):",
      dependents,
      "Length:",
      dependents?.length
    );
 ...
 */
  ...
  const renderDocumentsSection = (sectionData, color) => {
    ...
    console.log("📄 Documents API Data:", documents);
    console.log("📄 Documents Section Data:", sectionData);
   ...
   */
    const transformToDocumentStructure = (apiData) => {
      ...
      if (!apiData || (Array.isArray(apiData) && apiData.length === 0)) {
        console.log("📭 No document data from API, using sample data");
      ...
      console.log("🏗️ Final folder structure:", folderStructure);
