...
// Import the Colors component for dynamic color generation
...
const getDocumentStatus = (docValue, photoValue) => {
  const hasDoc = docValue && docValue !== "" && docValue !== "";
  const hasPhoto = photoValue && photoValue !== "" && photoValue !== "";
      ...
      >
        {value ? (
          typeof value === "string" ? (
            ...
            // For non-string values (chips, icons, etc.), center them
            <Box
              sx={{
                display: "flex",
                justifyContent: "center",
  ...
  const renderMinimalInfo = () => (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        >
          {
            // Find the name field or fallback
...
export const DependentCard = ({
  ...
  const renderMinimalInfo = () => (
    ...
    >
      ...
      >
        <DetailItem label="Age" value={dependent.dependent_age || ""} />
        <DetailItem
          label="Blood Group"
          value={dependent.dependent_blood_group || ""}
        />
        <DetailItem label="DOB" value={dependent.dependent_dob || ""} />
      </Box>
   ...
   */
  const renderTableStructure = () => {
    // Define all possible fields for the dependent
    const fields = [
      ...
      { label: "Mobile No", value: dependent.mobile || "" },
      { label: "Email", value: dependent.email || "" },
      ...
      { label: "Document Type", value: dependent.documenttype || "" },
    ...
    // Conditionally add disability details
    if (dependent.is_person_disabled === "Yes") {
      fields.push(
        { label: "Nature of Disability", value: dependent.natureofdisability },
        {
          ...
          value: dependent.percentageofdisability || "",
        }
      );
    }
...
export const ViewToggleButtons = ({ currentView, onViewChange }) => {
  ...
  const views = [
    // { key: 'professional', label: 'Professional View', icon: <Work /> },
  ];
...
// =============================================================================
// SPECIALIZED COMPONENTS FOR ADDRESS FIELDS
// =============================================================================
 ...
 */
// export const AddressDetailItem = ({ label, value, maxWidth = 320 }) => {
//   const theme = useTheme();
//   const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
...
//       {/* Value - Always allow wrapping for addresses */}
//       <Typography
//         variant="body2"
//         sx={{
//           color: "#111827",
 ...
 */
// export const AddressFieldsContainer = ({ addresses }) => {
//   const theme = useTheme();
//   const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
...
//           label={address.label}
//           value={address.value}
//           //maxWidth={350} // Wider for addresses
//         />
//       ))}
//     </Box>
//   );
...
// Add this at the end of your employeeEfileComponents.js file
export {
